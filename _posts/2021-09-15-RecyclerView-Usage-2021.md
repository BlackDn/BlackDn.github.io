...
不过因为之前我们只有一个Item，一个ViewHolder，所以我可以保证拿到的`ViewHolder`就是我自定义的`ViewHolderOne`，因此参数中的`holder`就是`ViewHolderOne`。而现在我们有两个Item，两个ViewHolder，我们不能确定拿到的ViewHolder是哪个（可能是`ViewHolderOne`，也可能是`ViewHolderTwo`），因此，这个方法里的ViewHolder应该是他们的父类`RecyclerView.ViewHolder`，然后根据`viewType`转型成对应的ViewHolder。
