...
但是如果直接用`DELETE`，**自增的字段**是不会重置的，还会继续自增。我们可以直接修改自增的字段，比如从1开始自增：
...
内连接也是默认的`JOIN`方式，如果我们看到SQL中没有写是什么`JOIN`，只是单纯的`JOIN...ON`，那就是内连接。
...
6. [mysql让自增的id重新从0开始的命令](https://blog.csdn.net/weixin_38106322/article/details/115489394)
