...
认识Flash的宝们可能会比较熟徐补间动画，补间动画就是由我们提供**关键帧（Key Frame）**，然后电脑根据两个关键帧的图像位置，进行差值运算，自动生成关键帧之间的帧。所以叫补间动画，自动补充中间的部分嘛。像我以前玩MMD，用的都是补间动画来实现骨骼的移动。    
