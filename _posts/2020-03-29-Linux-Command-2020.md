...
5. Ctrl + K：清空至行尾
6. Ctrl + C：终止正在执行的命令  
6. Ctrl + L：清除屏幕并将当前行移到页面顶部（相当于`clear`）
6. Ctrl + A：光标移至行首，不删除命令
6. Ctrl + E：光标移至行尾，不删除命令
6. Ctrl + D：关闭Shell会话
...
4. u：撤销上次操作（类似常用的ctrl+z）
4. ctrl+r：取消上次撤销（类似常用的ctrl+y）
